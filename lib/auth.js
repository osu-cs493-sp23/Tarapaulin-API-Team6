...
const key = process.env.JWT_KEY || "thisisasecretkey"
...
exports.generateAuthToken = function generateAuthToken(user){
    ...
    const payload = {sub: user.id, name: user.name, email: user.email, admin: user.admin}
...
exports.isAdminLoggedIn = function (req, res, next){
    ...
    try{
        ...
        req.isUserAdmin = payload && payload.admin? payload.admin : false;
        next()
    }catch(err){
        // If error, return 401 (unauthorized)
        res.status(401).json({
